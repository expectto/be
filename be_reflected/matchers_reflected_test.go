...
var _ = Describe("MatchersReflected", func() {
    ...
    <%T>: %v
to be kind of %s`, actual, actual, expected.String())))
		},
			...
			Entry("string", "1", reflect.Bool),
			Entry("bool", true, reflect.String),
			Entry("chan", make(chan int), reflect.Func),
			//Entry("func", func() {}, reflect.Chan),
			//Entry("map", map[string]int{}, reflect.Ptr),
			//Entry("ptr", new(int), reflect.Map),
		)
	})
})
